...
import (
	...
	"example/internal/entity/user"
	u_response "github.com/DVV-15324/witches/pkg/core/response_logger"
	u_hash "github.com/DVV-15324/witches/pkg/core/utils"
)

func (a *UsecaseAuth) UsecaseRegister(ctx context.Context, auth *entityAuth.Auth, name string) *u_response.ErrorResponse {
	// check tài khoản tồn tại
	authEmail, err := a.ReponsitoryAuth.GetAuthByEmail(ctx, auth.Email)
	if authEmail != nil {
		resp := u_response.NewErrorResponse(409, ErrorEmailIsExisted, time.Now())
		return resp
	}
	if err != nil {
