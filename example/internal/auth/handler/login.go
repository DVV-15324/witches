...
import (
	...
	mapping "example/internal/auth/mapping"
	"time"
...
func (h *AuthHandle) HandleLogin() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if err_valid != nil {
			...
			w_resp.WriteErrorWithLog(c, h.Core.Logger, h.Core.Config, resq)
			return
		}

		domain := mapping.FromLoginToDomainAuth(&data)

		// Use utils to get device info
		deviceID, ipAddress, userAgent, locale, timeZone := h.Core.DeviceHelper.GetDeviceInfo(c)

		claims, resq := h.UsecaseAuth.Login(c.Request.Context(), domain, deviceID, ipAddress, userAgent, locale, timeZone)
		if resq != nil {
			w_resp.WriteErrorWithLog(c, h.Core.Logger, h.Core.Config, resq)
			return
		}
