...
func (uc *RefreshUseCase) Create(ctx context.Context, token *domainRefresh.RefreshToken) *w_resp.AppError {
	if token == nil {
	...
	if token.ExpiresAt == 0 {
		token.ExpiresAt = time.Now().Unix() + uc.Core.Config.RefreshTokenTTL
	}
