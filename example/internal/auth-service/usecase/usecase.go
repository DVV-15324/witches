package usecase

import (
	"context"
	w_resp "github.com/DVV-15324/witches/pkg/core/response"
	w_utils "github.com/DVV-15324/witches/pkg/core/utils"
	"example/cmd/server/config"
	modelAuth "example/internal/shared/model"
	modelRefresh "example/internal/shared/model"
	modelUser "example/internal/shared/model"
)

type IAuthReponsitory interface {
	CreateAuth(ctx context.Context, auth *modelAuth.Auth) error
	GetAuthByEmail(ctx context.Context, email string) (*modelAuth.Auth, error)
	GetAuthByUserId(ctx context.Context, userID uint32) (*modelAuth.Auth, error)
}
type IRefreshTokenUsecase interface {
	Create(ctx context.Context, token *modelRefresh.RefreshToken) *w_resp.AppError
	GetByToken(ctx context.Context, token string) (*modelRefresh.RefreshToken, *w_resp.AppError)
	Revoke(ctx context.Context, token string, reason string, deviceID string) *w_resp.AppError
}

type IHash interface {
	GenerateFromPassword(password string, salt string) (string, error)
	CompareHashAndPassword(passwordStr string, password string, salt string) bool
}

type IUserUseCase interface {
	CreateUser(ctx context.Context, user *modelUser.User) (int, *w_resp.AppError)
	GetUserById(ctx context.Context, id int) (*modelUser.User, *w_resp.AppError)
}

type IJwt interface {
	ParseToken(ctx context.Context, tokenStr string) (*w_utils.JwtClaims, error)
	IssueAccessToken(ctx context.Context, sub string, tid string) (*w_utils.Token, error)
	IssueRefreshToken(ctx context.Context, sub string, tid string) (*w_utils.Token, error)
	IssueTokenPair(ctx context.Context, sub string, tid string) (*w_utils.TokenResponse, error)
}

type AuthUseCase struct {
	Config              *config.Config
	TxManager           w_utils.TxManager
	Jwt                 IJwt
	UserUseCase         IUserUseCase
	Hash                IHash
	AuthReponsitory     IAuthReponsitory
	RefreshTokenUseCase IRefreshTokenUsecase

	SessionService   *w_utils.SessionService
	BlacklistService *w_utils.BlacklistService
}

func NewAuthUseCase(
	jwt IJwt,
	userUseCase IUserUseCase,
	hash IHash,
	authReponsitory IAuthReponsitory,
	refreshTokenUseCase IRefreshTokenUsecase,
	txManager w_utils.TxManager,
	sessionService *w_utils.SessionService,
	blacklistService *w_utils.BlacklistService,
	config *config.Config,
) *AuthUseCase {
	return &AuthUseCase{
		Config:              config,
		Jwt:                 jwt,
		UserUseCase:         userUseCase,
		Hash:                hash,
		AuthReponsitory:     authReponsitory,
		RefreshTokenUseCase: refreshTokenUseCase,
		TxManager:           txManager,

		SessionService:   sessionService,
		BlacklistService: blacklistService,
	}
}
